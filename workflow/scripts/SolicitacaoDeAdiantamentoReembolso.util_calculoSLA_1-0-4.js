...
function montaTimeStamp(data, horario, tipoRetorno) {
	...
	// dia = data.split("")[8] + data.split("")[9];
	dia = data.split(" ")[0].split("/")[0];

	// mes = data.split("")[5] + data.split("")[6];
	mes = data.split(" ")[0].split("/")[1]+"";
	mes = parseInt(mes, 10);
	mes = mes;

	// ano = data.split("")[0] + data.split("")[1] + data.split("")[2] + data.split("")[3];
	...
	if (tipoRetorno == "string") {
		...
		dataRetorno = new Date(mesesIngles[parseInt(mes)] + " " + dia + ", " + ano + " " + horario);
...
function calculaPrazoConclusao(sla, expediente, atividadesDescartadas, primeirasAtividadesDescartadas, dataInicioParam, horaIncioParam, sequenceId) {
	...
	var objDataRetorno;
	try {
		objDataRetorno = hAPI.calculateDeadLineTime(dataInicio, segundosPrazoDepoisMeiaNoite, prazoMinutos, expediente, 1);
	} catch (error) {
		var xData = new Date();
		objDataRetorno = hAPI.calculateDeadLineTime(xData,37800,240,"Default",1);
	}
